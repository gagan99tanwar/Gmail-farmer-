...
def build_mirrored_keyboard(user_id, old_message):
    ...
    for row_index, row in enumerate(old_message.buttons):
        ...
        for col_index, old_button in enumerate(row):
            ...
            # Preserve the original old-bot button text/action internally.
            mapping[key] = {
            ...
            # Only change the text displayed on the new bot.
            display_text = old_button.text.replace("0.23$", "10₹")
            display_text = display_text.replace("0.15$", "6₹")
...
async def mirror_old_message(user_id, old_message, edit=False):
    ...
    # Preserve requested text replacement.
...
async def handle_message(
    ...
    # Help gets the requested response.
    ...
    # These two buttons are only meant to send their text in the chat.
    # Do not forward them to the old bot or generate another response.
    if message.text in (
        "➕ Register a new Gmail",
        "💰 Balance",
    ):
