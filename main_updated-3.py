...
def add_hold_amount(user_id, amount):
    USER_PENDING_HOLDS.setdefault(user_id, []).append({
        "amount": amount,
        "release_at": datetime.now(timezone.utc) + timedelta(days=3),
    })


def get_hold_totals(user_id):
    now = datetime.now(timezone.utc)
    holds = USER_PENDING_HOLDS.setdefault(user_id, [])
    on_hold = Decimal("0")
    ...
    for item in holds:
        ...
        if now >= release_at:
            withdrawable += amount
            # Keep released entries so their amount remains withdrawable.
            remaining.append(item)
        else:
            on_hold += amount
            remaining.append(item)

    USER_PENDING_HOLDS[user_id] = remaining

    # Withdrawable is calculated from released holds minus already withdrawn.
    released_total = sum(
        (item["amount"] for item in remaining if now >= item["release_at"]),
        Decimal("0"),
    )
    withdrawn = USER_WITHDRAW_STATES.get(
        user_id, {}
    ).get("withdrawn", Decimal("0"))
    withdrawable = released_total - withdrawn
    if withdrawable < 0:
        withdrawable = Decimal("0")

    return on_hold, withdrawable
...
async def old_bot_message_received(event):
    ...
    try:
        # A hold notice can also arrive as the direct response to an
        # inline-button click, so record it before display conversion.
        hold_amount = get_hold_amount_from_message(old_message.raw_text or "")
        if hold_amount is not None:
            add_hold_amount(user_id, hold_amount)
    ...
    for reply in replies:
        ...
        if reply.message:
            ...
            try:
                ...
                if hold_amount is not None:
                    add_hold_amount(chat_id, hold_amount)
...
def get_hold_amount_from_message(text):
    if not text or not text.endswith(HOLD_SUFFIX):
        return None

    first_line = text[:-len(HOLD_SUFFIX)].rstrip("\n").splitlines()[-1] if text[:-len(HOLD_SUFFIX)].strip() else ""
    mapping = {
        "0.23$ credited to hold": Decimal("10"),
        "0.23 credited to hold": Decimal("10"),
        "0.15$ credited to hold": Decimal("6"),
        "0.15 credited to hold": Decimal("6"),
    }
    return mapping.get(first_line)


def balance_status_markup():
    return BACK_BUTTON
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "⏳ Pending amount":
        on_hold, withdrawable = get_hold_totals(user_id)
        await message.reply_text(
            f"On hold amount: {format_rupees(on_hold)}\n"
            f"Withdrawable amount: {format_rupees(withdrawable)}",
            reply_markup=BACK_BUTTON,
        )
        return
    ...
    if text == "🏧 Withdraw":
        _, withdrawable = get_hold_totals(user_id)
        if withdrawable <= 0:
            await message.reply_text(
            ...
            return
        USER_WITHDRAW_STATES[user_id] = {
            "step": "amount",
            "withdrawn": USER_WITHDRAW_STATES.get(user_id, {}).get("withdrawn", Decimal("0")),
        }
        await message.reply_text(
    ...
    state = USER_WITHDRAW_STATES.get(user_id, {})

    if state.get("step") == "amount" and message.text:
        ...
        state["step"] = "qr"
        USER_WITHDRAW_STATES[user_id] = state
        await message.reply_text(
            "please send your QR code scanner",
            reply_markup=BALANCE_KEYBOARD,
        )
        return

    if state.get("step") == "qr":
        requested = state.get("requested", Decimal("0"))
        state["withdrawn"] = state.get("withdrawn", Decimal("0")) + requested
