...
async def handle_message(
    ...
    # Register must ALWAYS be relayed to the Old Bot.
    # If the user previously pressed Withdraw, the bot may still be
    # waiting for an amount. Do not let the Register button get parsed
    # as a withdrawal amount.
    if text_value == "➕ Register a new Gmail":
        ...
        logger.info(
            "Register request queued user=%s position=%s",
            user.id,
            relay_queue.qsize(),
        )
        return
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # /start resets any unfinished Withdraw flow.
    if update.effective_user:
        USER_WITHDRAW_STAGE[update.effective_user.id] = None
